        ...
        for i in range(self.L):
            for j in range(self.L):
                ...
                sample[:, :, i, j] = torch.bernoulli(
                    x_hat[:, :, i, j]).to(self.default_dtype_torch) * 2 - 1
...
def IsingM(sample):
    ...
    size, _, L, _ =sample.shape
    return np.sum(abs(np.sum(sample, axis=(1, 2, 3))))/(size*L*L)

def cal_IsingEnergy(sample):
    term = sample[:, :, 1:, :]*sample[:, :, :-1, :]
    term = term.sum(axis=(2, 3))
    output = term
